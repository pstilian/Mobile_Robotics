...
class Path:
    def __init__(self, val, cellNum, west, north, east, south):
        self.west = west
        self.north = north
        self.east = east
        self.south = south
...
class Cell:
    def __init__(self, west, north, east, south, cellNum, val, visited = False):
        ...
        # Store whether or not the cell has been visited before along with val and cellnum
        self.cellNum = cellNum
        self.val = val
...
def makeMap(goal, start):
    ...
    while GO:
        ...
        for q in mazeMap:
            if q.cellNum == newGoal:
                q.visited = True
                nVal = q.val + 1
                if q.north != 'W':
                    newNorth = q.cellNum - 4
                if q.south != 'W':
                    newSouth = q.cellNum + 4
                if q.east != 'W':
                    newEast = qCellNum + 1
                if q.west != 'W':
                    newWest = qCellNum - 1
        ...
        for r in mazeMap:
            if newNorth != 17:
                if r.cellNum == newNorth:
                    if r.val > nVal:
                        r.val = nVal
        for s in mazeMap:
            if newSouth != 17:
                if s.cellNum == newSouth:
                    if s.val > nVal:
                        s.val = nVal
        for t in mazeMap:
            if newEast != 17:
                if t.cellNum == newEast:
                    if t.val > nVal:
                        t.val = nVal
        for u in mazeMap:
            if newWest != 17:
                if u.cellNum == newWest:
                    if u.val > nVal:
                        u.val = nVal

        if (newNorth or newSouth or newEast or newWest) == start:
            GO = False
    
    while newStart != goal:
        newStart = 5
        #push direction into array
        for p in mazeMap:
            if p.cellNum == goal:
                direct.append("Stop")
                break
